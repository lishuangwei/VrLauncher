...
public class GetAppList {
    ...
    public ArrayList<AppBean> getLaunchAppList() {
        ...
        while (true) {
            ...
            Log.d("TAG","包名:"+noSeeApk);
            // 屏蔽自己 、芒果 、tcl新
            if (!noSeeApk.equals("com.cqsmiletv") && !noSeeApk.endsWith("com.starcor.hunan") && !noSeeApk.endsWith("com.xunixianshi.vrlanucher")) {
    ...
    public ArrayList<AppBean> getUninstallAppList() {
        ...
        while (true) {
            ...
            try {
                ...
                if ((mPackageInfo.applicationInfo.flags & mPackageInfo.applicationInfo.FLAG_SYSTEM) > 0) {//系统预装
                    localAppBean.setSysApp(true);
                } else if(!localAppBean.getPackageName().endsWith("com.xunixianshi.vrlanucher")&& !localAppBean.getPackageName().endsWith("com.koushikdutta.vysor")){
                    localArrayList.add(localAppBean);
                }
            } catch (NameNotFoundException e) {
                e.printStackTrace();
    ...
    public Drawable getAppIcon(String packageName){
        ...
        try {
            appIcon = localPackageManager.getApplicationIcon(packageName);
        } catch (NameNotFoundException e) {
            e.printStackTrace();
        }
        return appIcon;
