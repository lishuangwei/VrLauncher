package com.xunixianshi.vrlanucher.tvui.utils;
...
public class FileUtils {
	private static String[] videoType = {"MP4","3GP","AVI","MKV","WMV","MPG","VOB","FLV","SWF","MOV","RMVB","RM"};
	 ...
	 * E-mail:
	 * version:
	 * Created Time: 2017-10-07 : 09:42:13
	 * 遍历sdcard
	 **/
	private static void findFile(List<String> list,File p_file){
		for (File file : p_file.listFiles()) {
			if(file.isDirectory()){
				Log.d("BlueToothServer", "AbsolutePath = "+file.getAbsolutePath());
				if(!file.getAbsolutePath().contains("3box"+File.separator+"download")){
					findFile(list,file);
				}
			}else {
				String fileName = file.getName();
				Log.d("BlueToothServer", "fileName = "+fileName);
				String suffix = fileName.substring(fileName.lastIndexOf(".")+1).toUpperCase();
				if(Arrays.asList(videoType).contains(suffix) && file.length() >= 15*1024*1024){
	 ...
	 * 获取视频列表
	 */
	private static List<DownloadItem>  getLocalVideo(Context context) {
	 ...
	 * @param orderBy
	 */
	private static List<DownloadItem> getContentProvider(Context context,Uri uri,String[] projection, String orderBy) {
		// TODO Auto-generated method stub
		...
		while (mCursor.moveToNext()) {
			...
			if(mCursor.getString(3) != null){
				if(mCursor.getInt(3) >= 15*1024*1024){
					downloadItem.setType(1);
					downloadItem.setState(3);
					downloadItem.setName((mCursor.getString(1).indexOf(".") != -1)?mCursor.getString(1).substring(0,mCursor.getString(1).lastIndexOf(".")):mCursor.getString(1));
					downloadItem.setIcon(getVideoThumbnailSmallPath(mCursor.getString(2), DownloadUtil.imagePath));
					downloadItem.setUrl(mCursor.getString(2));
					downloadItem.setProcess(1.0f);
					downloadItem.setAppType(0);
					downloadItem.setVideoType(3);
	 ...
	 * 排除/sdcard/3box/download目录
	 **/
	public static List<DownloadItem> getLocalAllVideo(Context context){
		try{
			File parent = Environment.getExternalStorageDirectory();
			return findLocalFile(parent);//扫描文件
//			return getLocalVideo(context);//本地数据库获取
		}catch (Exception e){
	 ...
	 * 遍历sdcard
	 **/
	private static List<DownloadItem> findLocalFile(File p_file){
		List<DownloadItem> downloadItemList = new ArrayList<DownloadItem>();
		File imageFile = new File(DownloadUtil.imagePath);
		if(!imageFile.exists()){
			Log.d("TAG","创建图片目录");
			imageFile.mkdirs();
		}
		for (File file : p_file.listFiles()) {
			if(file.isDirectory()){
				Log.d("BlueToothServer", "AbsolutePath = "+file.getAbsolutePath());
				if(!file.getAbsolutePath().contains("3box"+File.separator+"download")){
					downloadItemList.addAll(findLocalFile(file));
				}
			}else {
				String fileName = file.getName();
				Log.d("BlueToothServer", "fileName = "+fileName);
				String suffix = fileName.substring(fileName.lastIndexOf(".")+1).toUpperCase();
				if(Arrays.asList(videoType).contains(suffix) && file.length() >= 15*1024*1024){
					MLog.d("~~~~~~~~~~~~~搜索到视频："+file.getAbsolutePath());
					DownloadItem downloadItem = new DownloadItem();
					downloadItem.setType(1);
					downloadItem.setState(3);
					downloadItem.setName((file.getName().indexOf(".") != -1)?file.getName().substring(0,fileName.lastIndexOf(".")):file.getName());
					downloadItem.setIcon(getVideoThumbnailSmallPath(file.getAbsolutePath(), DownloadUtil.imagePath));
					downloadItem.setUrl(file.getAbsolutePath());
					downloadItem.setProcess(1.0f);
					downloadItem.setAppType(0);
                    downloadItem.setVideoType(3);
					downloadItemList.add(downloadItem);
	...
	private static String getVideoThumbnailSmallPath(String videoPath,String imagePath) {
		String name = getFileNameToPath(videoPath);
		imagePath = imagePath + name;
		Log.d("TAG", "视频videoPath::" + videoPath);
		Log.d("TAG", "图片imagePath::" + imagePath);
		...
		File fpng = new File(imagePath + ".png");
		if (fpng.exists()) {
			Log.d("TAG", "直接返回");
			return fpng.getAbsolutePath();
		}
		File fbmp = new File(imagePath + ".bmp");
		if (fbmp.exists()) {
			Log.d("TAG", "直接返回");
			return fbmp.getAbsolutePath();
		}

		return saveBitmapFile(ThumbnailUtils.createVideoThumbnail(videoPath, MediaStore.Video.Thumbnails.MINI_KIND),f );
	}
	 ...
	 */
	public static String saveBitmapFile(Bitmap bitmap, File f) {
		Log.d("TAG", "保存图片");
		FileOutputStream fOut = null;
		try {
			//压缩bitmap
			bitmap = ThumbnailUtils.extractThumbnail(bitmap, 512, 287);
	...
	/*
    * Java文件操作  根据路径获取去掉后缀的文件名
    */
	private static String getFileNameToPath(String filename) {
